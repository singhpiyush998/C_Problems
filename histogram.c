...
void input_n(int* n)
{
    printf("How many collections are to be processed?\n");
    scanf("%d", n);
}

void input(Histogram* histogram)
{
    printf("Number of quiz scores that are to be entered?\n");
    scanf("%d", &histogram->score_count);

    printf("Enter the scores\n");
    for (int i = 0; i < histogram->score_count; i++)
    {
        scanf("%d", &histogram->scores[i]);
    }
    histogram->repeating_number_count = 0;
}

void input_n_histograms(int n, Histogram histograms[n])
{
    for (int i = 0; i < n; i++)
    {
        input(&histograms[i]);
...
int main()
{
    ...
    input_n_histograms(ques_count, histograms);
